d3.parcoords = function(config) {
    ...
    __.data.forEach(function(d) {
        __.gradientCache[d.ids] = {};
        // Loop through dimensions.
        d3.entries(__.dimensions).forEach(function(p, i) {
            __.gradientCache[d.ids][p.key] = {};
            for (let colorCombination of __.colorRange) {
                __.gradientCache[d.ids][p.key][colorCombination] = null;
            };
        });
    });
...
// getter/setter with event firing
    ...
    /**
     * Renders optimal and pessimal Pareto frontiers.
     * @param canvasContext Canvas context to use for rendering of Pareto frontiers.
     * @param min Minimum on y-axis to be considered.
     * @param max Maximum value on y-axis to be considered.
     */
    pc.renderParetoFrontiers = function(canvasContext, min, max)
    {
        let coordinates = [];
        let ctx         = pc.ctx[canvasContext];

        // Save context settings to restore them later.
        let ctxLineWidth    = ctx.lineWidth;
        let ctxStrokeStyle  = ctx.strokeStyle;
        let ctxGlobalAlpha  = ctx.globalAlpha;
        ...
        // Iterate dimension in sorted order.
        for (let dimKey in __.dimensions) {

            if (__.dimensions.hasOwnProperty(dimKey)) {
                // 1. Calculate coordinates.
                coordinates.push({
                    min: {
                        x: position(dimKey),
                        y: __.dimensions[dimKey].yscale(min)
                    },
                    max: {
                        x: position(dimKey),
                        y:__.dimensions[dimKey].yscale(max)
                    }
                });

                // 2. Draw lines from this to previous dimension.
                if (coordinates.length > 1) {
                    let dimIndex = coordinates.length - 1;

                    // Draw max. line.
                    ctx.moveTo(coordinates[dimIndex - 1].max.x, coordinates[dimIndex - 1].max.y);
                    ctx.lineTo(coordinates[dimIndex].max.x, coordinates[dimIndex].max.y);
                    ctx.stroke();

                    // Draw min. line.
                    ctx.moveTo(coordinates[dimIndex - 1].min.x, coordinates[dimIndex - 1].min.y);
                    ctx.lineTo(coordinates[dimIndex].min.x, coordinates[dimIndex].min.y);
                    ctx.stroke();
                }
            }
        }

        // Restore old ctx settings.
...
// returns the y-position just beyond the separating null value line
    ...
    function single_path(d, ctx) {
        ...
        let colors              = __.colors(d);
        // Store start coordinates for each point.
        ...
        d3.entries(__.dimensions).forEach(function(p, i) {  //p isn't really p
            ...
            if (i == 0) {
                // Make deep copy of coordinates for later use.
                startCoordinates = $.extend(true, [], coordinates);
                // Move to line's starting point.
                ctx.moveTo(coordinates[0], coordinates[1]);
            }
            else {
                // Initialize gradient, if not already done.
                if (__.gradientCache[d.ids][p.key][colors[i] + colors[i - 1]] === null) {
                    ...
                    __.gradientCache[d.ids][p.key][colors[i] + colors[i - 1]] = grad;
                }

                // Get cached gradient object.
                ctx.strokeStyle = __.gradientCache[d.ids][p.key][colors[i] + colors[i - 1]];
                // Draw line.
